<!-- <template>
    <div id="TopNav" class="fixed bg-white z-30 flex items-center w-full border-b h-[61px]">
        <div :class="route.fullPath === '/' ? 'max-w-[1150px]' : ''"
            ...
            <SearchUser endpoint="/api/usersearch/" placeholder="Search accounts"
                wrapper-class="hidden md:flex items-center bg-[#F1F1F2] p-1 rounded-full max-w-[380px] w-full"
                input-class="bg-transparent w-full" @select="goToProfile" />
...
<template>
    <div id="TopNav" class="fixed bg-white z-30 flex items-center w-full border-b h-[61px]">
        <div :class="route.fullPath === '/' ? 'max-w-[1150px]' : ''"
            ...
            <!-- Search -->
            <SearchUser :endpoint="searchEndpoint" placeholder="Search accounts"
                wrapper-class="hidden md:flex items-center bg-[#F1F1F2] p-1 rounded-full max-w-[380px] w-full"
                input-class="bg-transparent w-full" @select="goToProfile" />
            ...
            <div class="flex items-center justify-end gap-3 min-w-[275px] max-w-[320px] w-full">
                ...
                <div v-else class="flex items-center">
                    ...
                    <div class="relative">
                        <button class="mt-1" @click="showMenu = !showMenu">
                            <img class="rounded-full" width="33" :src="$userStore.image || '/default-avatar.png'" />
                        </button>
...
const searchEndpoint = '/api/usersearch/'
...
// Navigate to selected profile (triggered by SearchUser @select)
const goToProfile = (user) => {
    router.push(`/profile/${user.id}`)
}
